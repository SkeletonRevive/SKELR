...
//
// Bootup the masternode, look for a 30000 SKELR input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in SkeletonRevive.conf */
...
{
    CSkeletonRevivecoinAddress address(collateralAddress);
