...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    std::vector<unsigned char> result;
    CXIOScoinSecret secret;
    CXIOScoinAddress addr;
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CXIOScoinSecret::SetString tests isValid, whereas CXIOScoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CXIOScoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CXIOScoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CXIOScoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CXIOScoinAddressVisitor(&dummyAddr), nodest));
