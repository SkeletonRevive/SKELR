        ...
        if (mine != ISMINE_NO) {
          ...
          txnouttype whichType;
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CXIOScoinAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <XIOSaddress>\n"
            "Return information about <XIOSaddress>.");

    CXIOScoinAddress address(params[0].get_str());
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <XIOSpubkey>\n"
            "Return information about <XIOSpubkey>.");
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <XIOSaddress> <signature> <message>\n"
    ...
    CXIOScoinAddress addr(strAddress);
    if (!addr.IsValid())
