...
void Shutdown()
{
    ...
    RenameThread("SkeletonRevive-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: SkeletonRevive.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: SkeletonRevived.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSkeletonReviveamount=<n> " + _("Keep N SkeletonRevive anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SkeletonRevive is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SkeletonRevive is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SkeletonRevive version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SkeletonRevive server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SkeletonRevive") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SkeletonRevive to complete") << "\n";
    ...
    nAnonymizeSkeletonReviveAmount = GetArg("-anonymizeSkeletonReviveamount", 0);
    if(nAnonymizeSkeletonReviveAmount > 999999) nAnonymizeSkeletonReviveAmount = 999999;
    if(nAnonymizeSkeletonReviveAmount < 2) nAnonymizeSkeletonReviveAmount = 2;
    ...
    LogPrintf("Anonymize SkeletonRevive Amount %d\n", nAnonymizeSkeletonReviveAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1SKELR+1000 == (.1SKELR+100)*10
       10SKELR+10000 == (1SKELR+1000)*10
    */
