...
std::string getOutputs(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        CSkeletonRevivecoinAddress addressSource(source);
...
{    
    ...
    if(block == false) {
        ...
        ui->valueBox->setText(QValue + " SkeletonRevive");
        ui->txID->setText(QID);
        ui->outputBox->setText(QOutputs);
        ui->inputBox->setText(QInputs);
        ui->feesBox->setText(QFees + " SkeletonRevive");
    }
}
