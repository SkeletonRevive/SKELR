...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("SkeletonRevive"))
        return false;
...
{
    // Convert SkeletonRevive:// to SkeletonRevive:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("SkeletonRevive://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "SkeletonRevive:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SkeletonRevive.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SkeletonRevive.lnk
...
{
    return GetAutostartDir() / "SkeletonRevive.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a SkeletonRevive.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SkeletonRevive\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the SkeletonRevive app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add SkeletonRevive app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SkeletonRevive-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SkeletonRevive-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SkeletonRevive-Qt"));
