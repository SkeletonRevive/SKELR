...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSkeletonReviveAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSkeletonReviveAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSkeletonReviveAmount = strAnonymizeSkeletonReviveAmount.remove(strAnonymizeSkeletonReviveAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkeletonReviveAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSkeletonReviveAmount*COIN) nMaxToAnonymize = nAnonymizeSkeletonReviveAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSkeletonReviveAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSkeletonReviveAmount));
        strAnonymizeSkeletonReviveAmount = strAnonymizeSkeletonReviveAmount.remove(strAnonymizeSkeletonReviveAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkeletonReviveAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeSkeletonReviveAmount == 0){
