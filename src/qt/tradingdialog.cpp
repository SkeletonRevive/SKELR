    ...
    model(0)
{
    ...
    ui->SkeletonReviveAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexSkeletonReviveLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "SkeletonRevive(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "SkeletonRevive(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(SkeletonRevive)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //SkeletonRevivest get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=btc-SkeletonRevive");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-SkeletonRevive&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-SkeletonRevive&count=100");
      return Response;
}
...
QString tradingDialog::BuySkeletonRevive(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SkeletonRevive&quantity=";
...
QString tradingDialog::SellSkeletonRevive(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SkeletonRevive";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=SkeletonRevive";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SkeletonRevive&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>SkeletonRevive Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> SkeletonRevive");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double SkeletonReviveSupply = 0;
    double SkeletonReviveDemand = 0;
    double BtcSupply = 0;
    double BtcDemand = 0;
    ...
    ui->SkeletonReviveSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(SkeletonReviveSupply,'i',8) + "</span><b> SkeletonRevive</b>");
    ...
    ui->SkeletonReviveDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(SkeletonReviveDemand,'i',8) + "</span><b> SkeletonRevive</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("SkeletonRevive");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->SkeletonReviveAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("SkeletonRevive");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexSkeletonReviveLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("SkeletonRevive");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->SkeletonReviveBalanceLabel,*ui->SkeletonReviveAvailableLabel_2,*ui->SkeletonRevivePendingLabel, QString::fromUtf8("SkeletonRevive"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputSkeletonRevive->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("SkeletonRevive");
    ...
    double AvailableSkeletonRevive = BalanceObject["Available"].toDouble();
    ...
    QString DumpStr = "";
    QString TotalStr = "";
    if ( Qty < AvailableSkeletonRevive )
    {
...
{
    //calculate amount of BTC that can be gained from selling SkeletonRevive available balance
    QString responseA = GetBalance("SkeletonRevive");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableSkeletonRevive = ResultObject["Available"].toDouble();

    ui->UnitsInputSkeletonRevive->setText(str.number(AvailableSkeletonRevive,'i',8));
}
...
{
    double Quantity = ui->BittrexSkeletonReviveLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableSkeletonRevive,'i',8));
}
...
void tradingDialog::on_BuySkeletonRevive_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuySkeletonRevive(Order,Quantity,Rate);
...
void tradingDialog::on_SellSkeletonReviveBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputSkeletonRevive->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputSkeletonRevive->text();
            Msg += " SkeletonRevive @ ";
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellSkeletonRevive(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellSkeletonRevive(Order,y,x);
                ...
                QString SellResponse = SellSkeletonRevive(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" SkeletonRevive for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" SkeletonRevive for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputSkeletonRevive_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
