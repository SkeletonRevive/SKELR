...
int nAnonymizeSkeletonReviveAmount = 1000;
...
#else
    const char* pszModule = "SkeletonRevive";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SkeletonRevive
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SkeletonRevive
    // Mac: ~/Library/Application Support/SkeletonRevive
    // Unix: ~/.SkeletonRevive
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SkeletonRevive";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "SkeletonRevive.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "SkeletonRevived.pid"));
